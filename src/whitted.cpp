...
//    }
    ...
    Color3f Li(const Scene *scene, Sampler *samplerr, const Ray3f &ray) const {
            ...
            if (its.mesh->getBSDF()->isDiffuse()) {
                ...
                return samplePointandReturnL(scene, samplerr, ray, its);


            }else{
                Normal3f wi = its.shFrame.toLocal(-ray.d);
                BSDFQueryRecord query(wi, NULL, EDiscrete);
                if (samplerr->next1D()<0.95) {
                    Color3f sampleWeight = its.mesh->getBSDF()->sample(query, samplerr->next2D());
                    Vector3f normalOut;
                    normalOut = its.shFrame.toWorld(query.wo);

                    Ray3f rayOut(its.p,normalOut);

                    return sampleWeight[0]* Li(scene,samplerr, rayOut)/ 0.95;
                }else
                    return Color3f(0.f);
            };
        ...
        Color3f samplePointandReturnL(const Scene *scene, Sampler *samplerr, const Ray3f &ray, const Intersection &its) const {
            float emitterCount;
...
//            Lr *= lightPointSampledPDF* lightPointSampledPDF;
